...
func (a *Adapter) Start(ctx context.Context, request daemon.StartRequest) (daemon.RunRef, error) {
	...
	prompt += "Empty collections are []; unused strings are empty, booleans false, and optional objects null. work_item_failure fails the entire WorkItem. candidate_declined releases this Claim and suppresses this candidate generation only in this Daemon; other Daemons can still claim it. For infrastructure problems return runtime_failure with system=false (candidate-specific) or system=true (Harness/Provider-wide), with the business result null; never fabricate business failure.\n"
