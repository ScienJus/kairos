...
func TestWorkflowStartOverSummaryCopiesOnlyCurrentFailures(t *testing.T) {
	old := domain.TaskID("old")
	source := domain.WorkItem{ID: "source", Failure: &domain.WorkItemFailure{Kind: domain.FailureExecution, Message: "current Workflow failure"}, StartOverContext: "old summary https://example.com/old", RecoveryInstructions: "old operator instructions"}
	tasks := []domain.Task{
	...
	summary := workflowStartOverSummary(source, tasks)
...
func TestWorkflowStartOverSummaryUTF8Boundary(t *testing.T) {
	source := domain.WorkItem{ID: "source", Failure: &domain.WorkItemFailure{Kind: domain.FailureExecution}}
	budget := domain.MaxHistoryTextBytes - len(workflowStartOverSummary(source, nil))
	for _, reason := range []string{strings.Repeat("x", budget), strings.Repeat("x", budget+1), strings.Repeat("界", budget/3) + strings.Repeat("x", budget%3), strings.Repeat("界", budget/3) + strings.Repeat("x", budget%3+1)} {
		...
		summary := workflowStartOverSummary(source, nil)
