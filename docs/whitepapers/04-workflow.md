...
A node may create exactly `MaxTaskInstancesPerNode` Task instances. A transition attempting to create its next instance makes the WorkItem Failed, ends active Claims, and records the destination node ID and limit in the failure event. The source Submission and decision remain committed; no over-limit target Task is created. Exiting to another node at the boundary remains valid.

Human recovery prepares one transaction-local snapshot for the replacement batch, indexing per-node Task instance counts, source activations and incoming Task relations. Each successful replacement advances the count and next Task position before the next replacement is checked. The console computes its suggested limit with per-node counts of existing and required replacement instances. This does not add a total Task limit.

Ordinary expansion queries the waiting activation, target node count and next Task position through existing indexed columns instead of decoding all Task/Activation history for each edge. It retains the current transaction and duplicate-waiting guard, with no extra cache or schema. See [runtime measurements](../workflow-runtime-performance.md) for the benchmark and remaining history-read costs.

> Workflow fixes the guardrails. Executors make choices only at the decision points the definition leaves open.

Workflows with a failed WorkItem or current failed Tasks offer two Human actions: **Continue execution** keeps the current WorkItem, successful branches, waiting joins and pending reviews, creates new Task attempts for failed/interrupted work, and delivers only missing inputs from committed decisions. **Start over** creates a new WorkItem at the initial nodes, copying the original intent and pinned Workflow version plus a bounded failure summary and operator instructions. The source ends Failed and retains its execution history; remaining Claims end in the same transaction. A retry of A joins the successful B from the same activation; if both failed, both need successful replacements before C runs. No arbitrary-stage replay is offered. Continue retains per-node counts; retries count as new instances and may require a higher limit (at most 500). Start over has independent counts. Ended Claims never revive; rediscover and claim fresh work after recovery. Start over retains a source WorkItem reference and current failure summary, without scanning or copying earlier URLs, Artifacts, Reviews, Submissions, or recovery summaries. Scoped executors cannot read another WorkItem, so Humans must list external outcomes to reuse in the current instructions; external actions are not undone. Migration backfills historical failures as ordinary execution failures, preserving their messages without automatically reopening work. See the API reference for `/continue` and `/start-over`.

Workflow retries preserve the failed source Task; only the replacement can progress. Start-over provenance points to another WorkItem with the same Workflow Definition ID and version, and neither that provenance nor the Definition binding can be changed after creation.
