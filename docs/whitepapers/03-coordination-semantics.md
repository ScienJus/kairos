...
The candidate set defines only the current choice space. A person or agent can choose proactively, and a future Agent Daemon can automate the same role-aware choice. A Claim then establishes unique execution responsibility for one concrete actor.
