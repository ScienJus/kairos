...
| WorkItems | `GET/POST /api/v1/work-items`, `GET /api/v1/work-items/{id}/context`, `POST /completion`, `POST /acceptance`, `POST /cancellation`, `POST /resume`, `POST /restart`; Coordination Claims use `POST /{id}/coordination-claims`, `POST /{id}/coordination-claims/{claim_id}/heartbeat`, and `DELETE /{id}/coordination-claims/{claim_id}` |
