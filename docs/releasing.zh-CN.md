...
- 限制 Actions 权限。只有 Release workflow 需要 `contents: write` 来创建 GitHub Release。
...
5. 安装 GoReleaser 2.17.1 和 Syft 后构建本地 Release Snapshot：

   ```bash
   goreleaser release --snapshot --clean
   ```

6. 解压一个 Archive，确认 `kairos-server --version` 输出预期版本、控制台可以加载，并且 `checksums.txt` 与 Archive 匹配。
...
3. Release workflow 会构建 macOS 与 Linux 的 amd64、arm64 Archive，创建 SHA-256 Checksum 和 Archive SBOM，并发布 GitHub Release。
