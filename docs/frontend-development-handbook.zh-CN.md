...
- 后端执行上下文返回明确的 capability，例如 `can_decompose`。
...
- Workflow 声明预期 Artifact 时，完成表单同时支持上传托管文件和登记外部 URI；两种方式都必须先获得 Artifact ID，再随 Submission 一起提交，不能把本地文件名或临时 Blob URL 当作交付结果。响应丢失后的相同创建请求必须复用 Idempotency Key；选择新文件、修改 URI 或切换 Claim 时再生成新 Key。
