...
function WorkflowTaskDetails({ task }: { task: WorkflowTaskDefinition }) {
  const { t } = useI18n()
  return <section className="workflow-task-definition"><span>{t('selectedTask')}</span><h3>{task.Title}</h3><p>{task.Description || t('noDescription')}</p><dl><div><dt>{t('executor')}</dt><dd>{t(task.Executor)}</dd></div><div><dt>{t('role')}</dt><dd>{task.AllowedRoles.join(', ') || t('unrestricted')}</dd></div><div><dt>{t('executionPolicy')}</dt><dd>{t(task.Execution === 'optional' ? 'optionalTask' : 'requiredTask')}</dd></div><div><dt>{t('reviewPolicy')}</dt><dd>{t(task.ReviewPolicy === 'required' ? 'reviewRequiredPolicy' : task.ReviewPolicy === 'executor_decides' ? 'reviewExecutorDecides' : 'reviewNone')}</dd></div></dl><div><strong>{t('acceptance')}</strong><p>{task.AcceptanceCriteria || '—'}</p></div>{task.DefaultTags.length > 0 && <div className="definition-tags">{task.DefaultTags.map(tag => <span key={tag}>{tag}</span>)}</div>}</section>
}
