    ...
    <div className="library-layout">
      ...
      <article className="definition-page workflow-definition-page">
        ...
        {!selectionLoading && !selectionError && selected && <>
          ...
          <section className="workflow-summary"><div><span>{t('tasks')}</span><strong>{selected.graph.tasks.length}</strong></div><div><span>{t('connections')}</span><strong>{selected.graph.relations.length}</strong></div><div><span>{t('executionLimit')}</span><strong>{selected.graph.max_task_executions || 100}</strong></div></section>
