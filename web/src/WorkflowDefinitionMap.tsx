...
export function WorkflowDefinitionMap({ definition, selectedTaskID, onSelect }: {
  ...
  const { t } = useI18n()
  const { tasks: tasks, relations: relations, start_task_ids: starts } = definition.graph
  ...
  const edges: Edge[] = relations.filter(relation => taskIDs.has(relation.from_task_id) && taskIDs.has(relation.to_task_id)).map(relation => ({
    id: relation.id, source: relation.from_task_id, target: relation.to_task_id,
    markerEnd: { type: MarkerType.ArrowClosed }, ...workflowEdgePresentation(relation.from_task_id, relation.to_task_id, layout.isBackEdge(relation.from_task_id, relation.to_task_id)),
