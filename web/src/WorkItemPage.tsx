  ...
  return <>
    <section className="work-panel mobile-work">
      ...
      {context.data && <>
        ...
        {context.data.work_item.started_over_from_work_item_id && <details className="work-brief recovery-context"><summary>{t('startOverHistory')}</summary><a href={`/work-items/${context.data.work_item.started_over_from_work_item_id}`}>{t('sourceWorkItem')}</a><Brief label={t('startOverHistory')} value={context.data.work_item.start_over_context} /></details>}
...
function WorkflowFailureNotice({ identity, workItem, tasks, recoveryTasks, failedTasks, nodeTitle, definitionLimit, onStartOver }: { identity: Identity; workItem: WorkItem; tasks: Task[]; recoveryTasks: Task[]; failedTasks: Task[]; nodeTitle?: string; definitionLimit?: number; onStartOver: (id: string) => void }) {
  const { t } = useI18n()
  ...
  const continueExecution = useMutation({
    mutationFn: () => api.continueWorkflow(identity, workItem.id, workItem.version, Number(limit), instructions),
  ...
  const startOver = useMutation({
    mutationFn: () => api.startOverWorkflow(identity, workItem.id, workItem.version, instructions),
    onSuccess: work => { setAction(null); invalidate(); onStartOver(work.id) },
  })
  const show = (next: 'continue' | 'start-over') => { setLimit(String(minimum)); setInstructions(''); continueExecution.reset(); startOver.reset(); setAction(next) }
  return <section className="workflow-failure" role="status">
    ...
    <div>
      ...
      <p>{isLimit ? t('workflowLimitDetails', { node: nodeTitle || failure.workflow_task_id, count: failure.task_instances, limit: failure.limit }) : failure?.message || (failedTasks.length ? t('workflowTasksFailedBody') : t('failureReasonUnavailable'))}</p>
